...
func _on_factory_process(_time : float, _delta_time : float):
	if _new_accepted_object:
		var direction = 0
		if _block_control.get_signal_value("splitter_redirect_forward") > 2:
			direction += 1
		elif _block_control.get_signal_value("splitter_redirect_to_left") > 2:
			direction += 2
		elif _block_control.get_signal_value("splitter_redirect_to_right") > 2:
			direction += 4
